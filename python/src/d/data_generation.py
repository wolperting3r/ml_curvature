...
def generate_data(N_values, stencils, ek, neg, silent, geometry, smearing, usenormal, dshift, interpolate, gauss, stz_kappa, interpolate_lower, affinity):
    '''
    affinity = [affinity]
    ...
    # st_sz_kappa = 7 # 0,571428571
    # st_sz_kappa = 11 # 0,363636364
    # st_sz_kappa = 25 # 0,16
    st_sz_kappa = stz_kappa
    ...
    for n in range(N_values):
        ...
        if smearing:
            ...
            # a = ((1 + u()*(interpolate - 1)) if interpolate else 1)  # between 1 and interpolate
            # a = ((0.75 + u()*(interpolate - 0.75)) if interpolate else 1)  # between 0.75 and interpolate
            a = ((interpolate_lower + u()*(interpolate - interpolate_lower)) if interpolate else 1) 
    ...
    if not visualize:
        ...
        if geometry == 'ellipse':
            ...
            geom_str = '_cir'
        # Create file name
        file_name = os.path.join(parent_path, 'data', 'datasets', 'data_'+str(st_sz[0])+'x'+str(st_sz[1])+('_eqk' if equal_kappa else '_eqr')+('_neg' if neg else '_pos')+geom_str+('_smr' if smearing else '_nsm')+('_shift1' if dshift else '')+('' if usenormal else 'b')+('_ab'+str(interpolate_lower) if interpolate else '') +(f'_int{interpolate}' if interpolate else '')+('_g' if gauss else '')+'_'+str(stz_kappa)+ '.feather')
        print(f'File:\n{file_name}')
        # Export file
